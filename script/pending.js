...
async function callback({ api, event, getLang, Reply }) {
    ...
    if (input[0] == "deny" || input[0] == "d") {
        if (indexes.length == 0) return api.sendMessage(getLang("invalidIndexes"), event.threadID);
...
module.exports.run = async function({ api, event, getLang }) {
    try {
        ...
        if (pendingThread.length == 0) return api.sendMessage(getLang("pendingThreadListEmpty"), event.threadID);
